    ...
    public class CirMeshProcessor
    {
        ...
        public void ProcessMeshData(SceneData sceneData, NodeData[] boneNodes)
        {
            ...
            if (_catMesh.Joints != null)
            {
                ...
                {
                    // Map joint index to node index in the scene
                    _jointToNodeMap[i] = Array.IndexOf([.. sceneData.Nodes], boneNodes[i]);
                }
            }

            // Find nodes with SourceMeshIndex and create separate mesh objects for each primitive
            ...
            foreach (var parentNode in nodesToProcess)
            {
                ...
                // Clear the parent node's mesh reference - it will just be a container
                parentNode.SourceMeshIndex = null;
                parentNode.MeshIndex = null;

                // Create a separate mesh node for each mesh in the group
                for (int meshIdx = 0; meshIdx < meshGroup.Meshes.Count; meshIdx++)
                {
                    ...
                    if (meshData != null && meshData.Primitives.Count > 0)
                    {
                        ...
                        // Add this mesh node as a child of the parent
        ...
        private void CreateSkinData(SceneData sceneData)
        {
            ...
            foreach (var kvp in _jointToNodeMap.OrderBy(x => x.Key))
            {
                ...
                // The inverse bind matrix transforms from world space to bone local space
                // This is what allows the bone to "capture" vertices in its local space
        ...
        private MeshData CreateSingleMeshData(RDBCatMesh.Mesh cMesh, string baseName, int meshIndex)
        {
            ...
            foreach (var vertex in cMesh.Vertices)
            {
                ...
                {
                    vertexPosition = GetVertexBindPosePosition(vertex);
                    vertexNormal = GetVertexBindPoseNormal(vertex);
                }
                else
                {
                    vertexPosition = new Vector3(vertex.Position.X, vertex.Position.Y, vertex.Position.Z);
                    vertexNormal = new Vector3(vertex.Normal.X, vertex.Normal.Y, vertex.Normal.Z);
                }
        ...
        {
            // Map from the local joint index in the source data to the 
            // skin's joint array index (which references scene nodes)
            if (_jointToNodeMap.ContainsKey(localJointIndex))
            {
                // Find the position of this joint in the ordered joint list
        ...
        private Vector3 GetVertexBindPosePosition(RDBCatMesh.Vertex vertex)
        {
            if (_catMesh.Joints == null || _catMesh.Joints.Count == 0)
            ...
            return Vector3.Lerp(worldPos2, worldPos1, vertex.Joint1Weight);
        }

        private Vector3 GetVertexBindPoseNormal(RDBCatMesh.Vertex vertex)
        {
            ...
            var normal1 = Vector3.TransformNormal(normal, bone1GlobalTransform);
            var normal2 = Vector3.TransformNormal(normal, bone2GlobalTransform);

            return Vector3.Normalize(Vector3.Lerp(normal2, normal1, vertex.Joint1Weight));
        }
        ...
        public List<int> GetUsedMaterialIndices(SceneData sceneData)
        {
            ...
            foreach (var mesh in sceneData.Meshes)
            {
                foreach (var prim in mesh.Primitives)
                {
                    ...
                    {
                        int matIndex = prim.MaterialIndex.Value;
                        if (matIndex >= 0) materialIndices.Add(matIndex);
                    }
                }
            }
